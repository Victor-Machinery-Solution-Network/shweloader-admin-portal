...
export function RowActions({ category, linkedCount }: RowActionsProps) {
  ...
  const deleteDescription =
    linkedCount > 0
      ? <><strong>&ldquo;{category.name}&rdquo;</strong> will be moved to the trash.<br />There {linkedCount === 1 ? "is" : "are"} <strong>{linkedCount}</strong> sub {linkedCount === 1 ? "category" : "categories"} linked to this category that must be removed first.</>
